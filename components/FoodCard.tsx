    ...
    <TouchableOpacity onPress={onPress} style={{backgroundColor: colors.card, borderRadius:16, overflow:"hidden"}}>
      ...
      <View style={{padding:10}}>
        <View style={{flexDirection:"row", justifyContent:"space-between", alignItems:"center"}}>
          ...
          <TouchableOpacity onPress={onHeart}><Ionicons name={isFav ? "heart" : "heart-outline"} size={20} color={colors.accent} /></TouchableOpacity>
        </View>
        <Text style={{color: colors.textMuted, fontSize:12}} numberOfLines={1}>{item.region}</Text>
      </View>
    </TouchableOpacity>
  );
}
